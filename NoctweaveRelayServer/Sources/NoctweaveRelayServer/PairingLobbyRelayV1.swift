...
enum PairingLobbyRelayLimitsV1 {
    static let leaseIDBytes = 16
    static let leaseCapabilityBytes = 32
    static let maximumAnnouncementBytes = 12 * 1024
    static let minimumLeaseSeconds = 30
    static let maximumLeaseSeconds = 120
    static let maximumListings = 32

    static let advertisedRegistry: [String: UInt64] = [
...
struct PairingLobbyAcquireRequestV1: Codable, Equatable {
    let leaseID: Data
    let leaseCapability: Data
    ...
    func encode(to encoder: Encoder) throws {
        guard isStructurallyValid else {
            throw invalidModelEncoding(self, encoder, context: "Pairing lobby acquire request")
        }
...
struct PairingLobbyReleaseRequestV1: Codable, Equatable {
    let leaseID: Data
    let leaseCapability: Data

    init(leaseID: Data, leaseCapability: Data) {
        self.leaseID = leaseID
        self.leaseCapability = leaseCapability
    }
    ...
    func encode(to encoder: Encoder) throws {
        guard isStructurallyValid else {
            throw invalidModelEncoding(self, encoder, context: "Pairing lobby release request")
        }
        var values = encoder.container(keyedBy: CodingKeys.self)
        try values.encode(leaseID, forKey: .leaseID)
        try values.encode(leaseCapability, forKey: .leaseCapability)
...
struct PairingLobbyListRequestV1: Codable, Equatable {
    init() {}
...
struct PairingLobbyLeaseV1: Codable, Equatable {
    let leaseID: Data
    let announcement: Data
    let expiresAt: Date

    init(leaseID: Data, announcement: Data, expiresAt: Date) {
        self.leaseID = leaseID
        self.announcement = announcement
    ...
    func encode(to encoder: Encoder) throws {
        guard isStructurallyValid else {
            throw invalidModelEncoding(self, encoder, context: "Pairing lobby lease")
        }
        var values = encoder.container(keyedBy: CodingKeys.self)
        try values.encode(leaseID, forKey: .leaseID)
        try values.encode(announcement, forKey: .announcement)
