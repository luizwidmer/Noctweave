# Noctweave relay operator reference

[← Setup and project overview](README.md)

Run command examples from the root of the Noctweave checkout, not from
`NoctweaveRelayServer/`. The executable's `--help` is the authoritative option
list for the build you are running.

[Protocol surface](#protocol-surface) · [Noctweave Net relay roles](#noctweave-net-relay-roles) · [Authenticated federation and naming](#authenticated-federation-and-naming) · [Calls through coturn](#calls-through-coturn) · [Transports](#transports) · [Opaque routes](#opaque-routes) · [Rendezvous transport](#rendezvous-transport) · [Same-relay pairing discovery](#same-relay-pairing-discovery) · [Encrypted blobs](#encrypted-blobs) · [App-neutral realtime modules](#app-neutral-realtime-modules) · [Federation](#federation) · [Operator console](#operator-console) · [Optional privacy advertisements](#optional-privacy-advertisements) · [Common secrets](#common-secrets)
