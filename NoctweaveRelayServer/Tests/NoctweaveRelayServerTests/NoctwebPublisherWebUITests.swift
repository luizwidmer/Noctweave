...
final class NoctwebPublisherWebUITests: XCTestCase {
    ...
    func testPublisherScriptPreservesIdentityHostingAndSandboxBoundaries() throws {
        ...
        XCTAssertTrue(html.contains("class=\"brand-plane-upper\""))
        XCTAssertFalse(html.contains("aria-hidden=\"true\">N</span>"))
        XCTAssertFalse(html.contains(#"name="color-scheme" content="dark""#))
        XCTAssertTrue(html.contains(#"id="appearanceSelect"#))
        XCTAssertTrue(html.contains(#"id="ctaEnabledInput"#))
        XCTAssertTrue(html.contains(#"id="publicationCard" aria-labelledby="hostedHeading" hidden"#))
        XCTAssertTrue(html.contains("Reset local draft…"))
        XCTAssertTrue(html.contains(#"id="vaultDialog""#))
        XCTAssertTrue(html.contains(#"id="vaultPassword" type="password""#))
        ...
        XCTAssertTrue(css.contains("--button-bg: #922d35"))
        XCTAssertTrue(css.contains("--button-bg: #c96a61"))
        XCTAssertTrue(css.contains("--brand-upper: #922d35"))
        XCTAssertTrue(css.contains("--brand-upper: #faf3ea"))
        XCTAssertTrue(css.contains("[hidden] { display: none !important; }"))
        XCTAssertTrue(css.contains(".topbar-actions { width: 100%; display: grid"))
        XCTAssertTrue(css.contains(".topbar-actions .primary { white-space: nowrap; }"))
        XCTAssertFalse(css.contains("#7c6cff"))
        XCTAssertFalse(css.contains("#9588ff"))
        XCTAssertFalse(css.contains("#5b49ed"))
        XCTAssertFalse(script.contains("allow-same-origin"))
        XCTAssertTrue(script.contains("noctweave.publisher.appearance"))
        XCTAssertTrue(script.contains("elements.publicationCard.hidden = !hosted"))
        XCTAssertTrue(script.contains("project.ctaEnabled && safeHTTPURL"))
        XCTAssertTrue(script.contains(#"localStorage.removeItem("noctweave.publisher.appearance")"#))
        XCTAssertFalse(script.contains("localStorage.setItem"))
        XCTAssertFalse(script.contains("localStorage.getItem"))
        XCTAssertTrue(script.contains("PBKDF2"))
        XCTAssertTrue(script.contains("VAULT_ITERATIONS = 600000"))
        XCTAssertTrue(script.contains(#"createObjectStore("vault")"#))
        XCTAssertFalse(script.contains(#"createObjectStore("projects")"#))
        XCTAssertFalse(script.contains(#"createObjectStore("identities")"#))
        XCTAssertFalse(script.contains(#"createObjectStore("publications")"#))
        XCTAssertTrue(script.contains("Publisher vault changed in another tab"))
