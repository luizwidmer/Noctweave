...
test("relay desktop packages source and keeps Docker and admin boundaries explicit", async () => {
  const [config, backend, html] = await Promise.all([
  ...
  expect(config).toMatch(/"Dockerfile":\s*"relay-source\/Dockerfile"/);
  expect(config).toMatch(/"Sources":\s*"relay-source\/Sources"/);
