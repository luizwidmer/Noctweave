...
| SR-06 | Burn must remove live local authority. | Remove the burned persona record, relationships, groups, sessions, pending operations, and local route capabilities; reject pre-burn asynchronous construction through a process-local, non-serializable scope token; host-anchored browser state must enter a fixed-slot terminal burn generation before relay cleanup; never retain a recoverable archived identity. |
...
| SR-29 | An alternate carrier must not become a protocol downgrade or an unbounded proxy. | Carry exact relay bytes; preserve ML-DSA relay verification and all end-to-end controls; pin or authenticate the carrier address; bind client gateways to loopback; fix and allow-list server upstreams; reject redirects; enforce request, response, timeout, concurrency, per-peer, and global bounds; never retry uncertain operations in the carrier adapter. |
