...
| `liboqs` | Docker build from `open-quantum-safe/liboqs` | `0.16.0`, commit `5a1a854b0dc9f2141bdc771c555ee60c37950183` | The immutable commit is checked after fetch before compilation. Required for Linux actor-proof and coordinator-signature verification. |
...
It proves liboqs 0.15.0-to-0.16.0 bidirectional ML-KEM/ML-DSA
interoperability, generates a candidate SBOM and verifies the checked-in
snapshot is current, checks package resolution, runs the Linux relay test
suite, and runs optional Docker/Trivy checks when those tools are installed.
...
5. Prove that existing 0.15.0 identities and KEM material remain compatible:

```bash
scripts/test-liboqs-interop.sh
```

The matrix generates ML-DSA and ML-KEM keypairs under both releases. Each
release signs with the other's private-key encoding, verifies the other's
signatures, encapsulates to keys from both releases, and decapsulates the
resulting ciphertexts with the opposite release.

6. Rebuild the Apple XCFramework from the reviewed source checkout:
...
7. Build the Docker image with the reviewed `liboqs` version and immutable commit:
...
8. Record final artifact hashes:
...
9. Review upstream security advisories for:
   - SwiftNIO
   - Swift Crypto
   - Open Quantum Safe `liboqs`
   - Docker base image packages

10. Review `noctweave_core_stability_policy.md` and record any source, CLI,
