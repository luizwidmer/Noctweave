...
**Date:** June 24, 2026  
**Scope:** Noctyra client, `NoctweaveCore`, macOS relay, Linux relay, federation discovery, relay forwarding, and relay discovery research.
...
- Mitigations present: ML-KEM/ML-DSA session setup, AEAD payloads, authenticated inbox fetch/ack, temporal buckets, attachment TTL, relay password support, TLS/WSS support, and ciphertext-only prefetch staging that defers acknowledgement until unlocked sync. Apple helper prefetch uses delegated inbox-access keys for direct ciphertext only, does not store the long-term identity signing key, identity display names, identity fingerprints, group routing metadata, or helper status counts, rewrites successfully decoded stale helper configs and statuses that still contain removed sensitive fields or count-bearing messages, and caps helper profiles, per-profile relay responses, staged ciphertext records, and encrypted helper file size.
...
- `NoctweaveCore/Sources/NoctweaveCore/RelayServer.swift` advertises coordinator snapshot state and `knownOpenPeers` peer hints through `/info` responses.
- `NoctweaveCore/Sources/NoctweaveCore/OpenFederationDHTGatewayTransport.swift` provides the sidecar gateway client used to publish/query signed records without trusting raw public-network output.
- `NoctweaveCore/Sources/NoctweaveCore/OpenFederationDHTDiscovery.swift` and `OpenFederationDHTRecord.swift` provide the same signed-record validation, TTL, host cap, and total cap boundary.

Verified Linux relay support:
- `Noctweave Relay Server/Sources/NoctweaveRelayServer/RelayHandler.swift` advertises coordinator snapshot state and `knownOpenPeers` peer hints through relay info responses.
- `Noctweave Relay Server/Sources/NoctweaveRelayServer/OpenFederationDHTGatewayTransport.swift` mirrors the sidecar gateway client behavior.
- `Noctweave Relay Server/Sources/NoctweaveRelayServer/OpenFederationDHTDiscovery.swift` and `OpenFederationDHTRecord.swift` mirror the signed-record validation boundary.
...
- `NoctweaveCore/Sources/NoctweaveCore/PublicRelayEndpointPolicy.swift`
...
- `Noctweave Relay Server/Sources/NoctweaveRelayServer/PublicRelayEndpointPolicy.swift`
  - Same parity fix for Linux relay.

- Regression tests:
  - `NoctweaveCoreTests.testPublicRelayEndpointPolicyRejectsIPv6TransitionPrivateTargets`
  - `RelayTCPIntegrationTests.testPublicRelayEndpointPolicyRejectsIPv6TransitionPrivateTargets`

- `NoctweaveCore/Sources/NoctweaveCore/OpenFederationDHTRecord.swift`
...
- Regression tests:
  - `NoctweaveCoreTests.testOpenFederationDHTRecordValidatesSignedRelayAdvertisement`
  - `NoctweaveCoreTests.testOpenFederationDHTRecordRejectsTamperedEndpoint`
  - `NoctweaveCoreTests.testOpenFederationDHTRecordRejectsNamespaceMismatch`
  - `NoctweaveCoreTests.testOpenFederationDHTRecordRejectsExpiredAndOverlongRecords`
  - `NoctweaveCoreTests.testOpenFederationDHTRecordRequiresSecureHttpOrWebSocketEndpoint`

- `NoctweaveCore/Sources/NoctweaveCore/OpenFederationDHTDiscovery.swift`
...
- `NoctweaveCore/Sources/NoctweaveCore/OpenFederationDHTGatewayTransport.swift`
...
- `Noctweave Relay Server/Sources/NoctweaveRelayServer/OpenFederationDHT*.swift`
...
- Regression/simulation tests:
  - `NoctweaveCoreTests.testOpenFederationDHTDiscoveryIsFeatureGated`
  - `NoctweaveCoreTests.testOpenFederationDHTDiscoveryAcceptsValidatedSignedRecords`
  - `NoctweaveCoreTests.testOpenFederationDHTDiscoveryRejectsPoisonedRecords`
  - `NoctweaveCoreTests.testOpenFederationDHTDiscoveryCapsHostFloods`
  - `NoctweaveCoreTests.testOpenFederationDHTDiscoveryCapsTotalRecords`
  - `NoctweaveCoreTests.testOpenFederationDHTDiscoveryHandlesChurnAndStaleRecords`
  - `NoctweaveCoreTests.testOpenFederationDHTDiscoveryEnginePublishesAndQueriesBehindFeatureFlag`
  - `NoctweaveCoreTests.testOpenFederationDHTDiscoveryEngineDisabledDoesNotTouchTransport`
  - `NoctweaveCoreTests.testOpenFederationDHTDiscoveryEngineRejectsInvalidLocalAdvertisementBeforePublish`
  - `NoctweaveCoreTests.testOpenFederationDHTDiscoveryEngineHonorsTransportQueryLimit`
  - `NoctweaveCoreTests.testOpenFederationDHTHTTPGatewayTransportPublishesWithAuthHeader`
  - `NoctweaveCoreTests.testOpenFederationDHTHTTPGatewayTransportQueriesRecords`
  - `NoctweaveCoreTests.testOpenFederationDHTHTTPGatewayTransportRejectsOversizedResponse`
  - `NoctweaveCoreTests.testOpenFederationDHTHTTPGatewayRefreshAppliesPoisoningAndFloodGuards`
...
- `NoctweaveCore/Sources/NoctweaveCore/FederationDirectorySignature.swift`
...
- `Noctweave Relay Server/Sources/NoctweaveRelayServer/FederationDirectorySignature.swift`
...
- Regression tests:
  - `NoctweaveCoreTests` coordinator directory snapshot verification paths now exercise ML-DSA-65 signatures.
...
- `Noctweave Documentation/relay_ops_hardening_guide.md`
...
- `Noctweave Documentation/dependency_sbom_and_release_policy.md`
  - Adds a maintained dependency inventory, Docker build input inventory, dependency audit steps, and release signing policy.

- `scripts/generate-sbom.py` and `Noctweave Documentation/noctyra_sbom.json`
...
- `NoctweaveCore/Sources/NoctweaveCore/RelayStore.swift`
...
- `Noctweave Relay Server/Sources/NoctweaveRelayServer/RelayStore.swift`
  - Adds Linux relay parity for the same normalized transactional SQLite schema and row-scoped corruption tolerance.

- Regression tests:
  - `NoctweaveCoreTests.testRelayStoreDiskPersistenceSkipsCorruptNormalizedMessageRow`
  - `RelayStoreParityTests.testDiskPersistenceSkipsCorruptNormalizedMessageRow`

- `Noctweave Relay Server/Sources/NoctweaveRelayServer/LineCodec.swift`
...
- `Noctweave Relay Server/Sources/NoctweaveRelayServer/RelayHandler.swift` and `HTTPRelayBridge.swift`
...
2. **Network anonymity remains out of scope**
   - Current: metadata reduction, replicated XOR-PIR primitives under a non-collusion assumption with padded fixed-bucket query shares, fixed-size response shares, fail-closed PIR plan-integrity validation, onion packet primitives, fixed-size mixnet packet padding, mixnet batch/cover scheduling, bounded continuous cover-cycle planning, deterministic inter-relay cover coordination plans, deterministic diverse route selection, mixnet route-policy validation, and encrypted ciphertext-only direct prefetch persistence for OS-permitted helper fetch paths without exposing the long-term identity signing key, identity names, identity fingerprints, group routing metadata, or helper status counts, including stale-helper-config/status scrubbing after successful decode and bounded helper work queues.
...
- Run `swift test` in `NoctweaveCore`.
- Run `swift test` in `Noctweave Relay Server`.
