...
- Core wake planning now has a bounded prefetch execution gate: relay-advertised wake cycle plans are converted into explicit per-cycle profile limits, per-profile envelope limits, long-poll envelope limits, and total staged-envelope limits before a helper path can fetch.
- Manual federation with explicit standard-relay node lists, plus curated federation with allow-list, coordinator directory, quorum, and signed snapshot controls.
- Open federation release profile based on coordinator snapshots, bounded peer exchange, explicit relay-native DHT node mode, and optional DHT gateway integration, not autonomous public DHT participation. Focused verification covers coordinator registration/listing with signed snapshots, curated forwarding through coordinator directory policy, relay-native DHT publish/list routes, cached-node fallback after transient gateway or peer-query failure, stale-cache eviction, and release failure if BEP5/libp2p/Kademlia autonomous adapter code appears in shipped source paths.
...
- App release-origin trust is delegated to the App Store signing and review path. Noctyra does not add custom app-provenance prompts or user-facing release verification loops for App Store builds. Repository checks focus on protocol correctness, dependency inventory, package pins, relay tests, and optional scanner hooks.
- `scripts/verify-whitepaper-alignment.sh` runs focused checks for metadata timestamp bucketing, root-ratchet visible timestamp bucketing, relay pairing timestamp bucketing on core and Linux relay stores, hidden-retrieval cover-query safeguards, decentralized wake cycle planning, ciphertext-only prefetch staging, group-ratchet distribution validation and stale-state recovery, bounded group protocol model checking, coordinator-backed federation routing, open-federation fallback/gateway simulation, and Linux relay parity.
...
- Group ratchet recovery fails closed when retained epoch history is empty, duplicated, transcript-broken, non-contiguous, missing the current commit, or carrying a retained distribution that does not match its commit metadata.
...
- Core mixnet packet padding now normalizes compatible outbound payloads to a configured fixed size and rejects malformed padded packets before opening.
...
- This adds fixed-packet, cover-packet, batching, inter-relay cover-plan, and capability-gating machinery, but still does not prove full mixnet deployment because live continuous network cover execution, shared route selection deployment, and network-wide latency policy remain outside the current implementation.
...
- No full mixnet deployment. The implementation now has onion packet, fixed-size mixnet packet padding, single-batch mixnet scheduling, bounded continuous cover-cycle planning, deterministic inter-relay cover coordination plans, deterministic route selection with operator/host diversity checks, and relay advertisement, but does not provide live network-wide cover execution or network-wide latency scheduling.
...
- No autonomous public DHT release mode; relay-native DHT node mode is bounded to signed short-lived relay records over the Noctyra relay protocol, while public-network adapters remain deferred until poisoning, churn, flooding, and operator-risk controls are externally validated. The local whitepaper verifier enforces this boundary by rejecting shipped source paths that introduce BEP5/libp2p/Kademlia adapter code.
- No centralized push-notification server by design, so closed-app instant delivery remains out of scope. Compatible pull, intent, or long-poll clients can stage encrypted ciphertext for later unlocked processing.
- Closed-app compatible staging now has an explicit core prefetch batch type and caller-keyed encrypted batch store. It stages sealed envelopes with deferred acknowledgements, rejects records that would acknowledge relay delivery, fails closed on wrong keys or corrupted stored batches, and keeps raw helper storage from containing plaintext batch data or sealed envelope bytes. The Apple client uses the same encrypted batch model for prefetched direct ciphertext instead of keeping separate app-specific envelope caches, excludes the long-term identity signing key, identity display names, identity fingerprints, group IDs, and group inbox routes from helper config, rewrites successfully decoded stale helper configs when legacy sensitive fields are still present, rewrites successfully decoded stale helper statuses when count-bearing fields or messages are still present, caps helper profiles, per-profile relay responses, and staged ciphertext records, and rejects oversized encrypted helper writes. Core wake-to-prefetch execution planning now provides an explicit bounded work plan before helper fetch attempts. Its App Intent response and persisted helper status are metadata-blind and do not report fetched-envelope, pending-envelope, or failed-profile counts outside the unlocked app.

## Alignment Summary
- **Aligned**: PQ identity, PQ session establishment, prekey handshake, ratcheting, rotation/burn continuity, relay-backed messaging, authenticated relay state changes, attachment controls, relay metadata, TLS deployment modes, coordinator-assisted federation, temporal-bucket timestamp minimization, fixed-size message-size buckets, fixed-size hidden-retrieval cover-query safeguards, replicated XOR-PIR primitive, padded query-share support, fixed response-slot support, PIR plan-integrity validation, replica-set validation support, operational PIR profile validation, PIR promotion evidence gating, onion packet primitive support, fixed-size mixnet packet padding, mixnet batch/cover scheduling primitive support with bounded cover-cycle planning, deterministic inter-relay cover plans, deterministic diverse route selection, and route-policy validation, decentralized wake cycle planning, bounded wake-to-prefetch execution planning, explicit ciphertext-only prefetch staging with deferred acknowledgements, encrypted ciphertext-only prefetch persistence, Apple-client direct prefetch batch storage without identity signing keys, identity names, identity fingerprints, or group routing metadata plus stale-config scrubbing and helper resource bounds, metadata-blind App Intent prefetch responses and persisted helper status, group-ratchet epoch-secret distribution validation, retained group epoch-history chain validation, direct group-ratchet skipped-epoch rejection, fail-closed retained-epoch recovery, retained-history fault-injection coverage, bounded group-state model checking, Linux relay parity for the same group and retrieval checks, release verifier enforcement against autonomous public-DHT adapters, repository-owned whitepaper verification checks, and App Store handled app distribution.
- **Partially aligned**: anonymity-strength metadata protection, PIR-class hidden retrieval, MLS-class group cryptography, autonomous open federation, and closed-app delivery. Current controls now enforce deterministic bucketing, fixed ciphertext-size buckets for message bodies, exact cover-response validation, signed registry commits, MLS epoch state, retained epoch-history chain validation, group-context AEAD binding, structurally validated ML-KEM member shares, retained epoch replay, direct skipped-epoch rejection in group ratchet state, bounded group-state model checking, auditable wake scheduling, explicit ciphertext-only direct/group staging with deferred acknowledgements, encrypted ciphertext-only prefetch persistence, replicated XOR-PIR under a non-collusion assumption with fixed-size padded query shares, response slots, and fail-closed plan-integrity validation, validated independent replica-set metadata, and weak-advertisement suppression, fail-closed onion route-privacy advertisement, fixed-size mixnet packet padding, deterministic mixnet batch/cover scheduling with bounded continuous cover-cycle plans, deterministic inter-relay cover plans, route selection with TLS/operator/host diversity checks, and fail-closed mixnet route-policy advertisement, but these do not claim strong anonymity, single-server cryptographic PIR, formal MLS proofs, public DHT release readiness, full mixnet deployment, or guaranteed background delivery.
...
- No repository-owned protocol alignment target is open after this pass; the focused verifier is the acceptance check for future protocol changes that touch metadata minimization, hidden retrieval, decentralized wake, group recovery, or federation routing.
- Remaining alignment work requires scope outside the current repository implementation: independent external audit, real device-lab fault injection, full network-wide mixnet deployment, single-server cryptographic PIR research, or an explicit future decision to re-open autonomous public-DHT release work.
