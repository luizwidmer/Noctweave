...
This document records the current software bill of materials and the minimum release checks for Noctyra client, core, and relay builds. It is a project-maintained SBOM, not an independent vulnerability assessment. The native machine-readable snapshot is `noctyra_sbom.json`, and the interoperable CycloneDX JSON export is `noctyra_cyclonedx_sbom.json`; both are generated by `scripts/generate-sbom.py`.

## Scope

- `NoctweaveCore`
- `Noctweave Relay Server`
- `Noctyra Messaging Client`
- `Noctyra Relay`
- Docker relay image
...
| --- | --- | --- | --- |
| `liboqs.xcframework` | `NoctweaveCore/Vendor/liboqs.xcframework` | vendored binary | Used by Apple-platform core/client/server builds for ML-KEM/ML-DSA operations. Treat replacement as a cryptographic supply-chain event. |
...
git diff --exit-code "Noctweave Documentation/noctyra_sbom.json"
git diff --exit-code "Noctweave Documentation/noctyra_cyclonedx_sbom.json"
```
