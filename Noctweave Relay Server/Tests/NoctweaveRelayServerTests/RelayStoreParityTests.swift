...
final class RelayStoreParityTests: XCTestCase {
    ...
    func testRelayInfoSuppressesWeakReplicatedPIRAdvertisement() {
        ...
        let issues = HiddenRetrievalPIRReplicaSetValidator.issues(for: weakReplicas)
        XCTAssertTrue(issues.contains(.duplicateHost))
        XCTAssertNil(RelayConfiguration(hiddenRetrieval: weakReplicas).makeInfo().hiddenRetrieval)
    }
    ...
    func testRelayInfoCarriesOptionalMixnetTransportSupport() {
        ...
        let configuration = RelayConfiguration(onionTransport: onion, mixnetTransport: support)
        let info = configuration.makeInfo()
    ...
    func testRelayInfoSuppressesMisleadingMixnetAdvertisement() {
        ...
        XCTAssertNil(RelayConfiguration(mixnetTransport: mixnet).makeInfo().mixnetTransport)
    ...
    private func overwriteMailboxEnvelope(at sqliteURL: URL, envelopeId: UUID, with data: Data) throws {
        ...
        guard sqlite3_open(sqliteURL.path, &db) == SQLITE_OK, let db else {
            throw NSError(domain: "NoctweaveRelayServerTests.SQLite", code: 5)
        }
        ...
        guard sqlite3_prepare_v2(db, sql, -1, &statement, nil) == SQLITE_OK, let statement else {
            throw NSError(domain: "NoctweaveRelayServerTests.SQLite", code: 6)
        }
        ...
        guard bindBlobResult == SQLITE_OK else {
            throw NSError(domain: "NoctweaveRelayServerTests.SQLite", code: 7)
        }
        guard sqlite3_bind_text(statement, 2, envelopeId.uuidString, -1, unsafeBitCast(-1, to: sqlite3_destructor_type.self)) == SQLITE_OK else {
            throw NSError(domain: "NoctweaveRelayServerTests.SQLite", code: 8)
        }
