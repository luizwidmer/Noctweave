...
- [x] Align OS-permitted wake behavior into a bounded execution gate: core converts relay-advertised wake cycle plans into capped prefetch execution plans with explicit profile-count, per-profile envelope, long-poll envelope, and total-cycle envelope limits. This item is closed because verifier-covered limits reject unbounded helper work. Do not track future platform tuning here; any new wake-behavior work must be a separate TODO naming the platform, trigger condition, acceptance test, and stop condition.
