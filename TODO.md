...
- [x] Align closed-app helper resource bounds: helper prefetch profiles and staged ciphertext records are capped before persistence/execution, and encrypted helper files now fail closed if a write would exceed the configured local size bound.
