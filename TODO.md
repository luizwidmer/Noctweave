...
- [x] Align OS-permitted wake behavior into a bounded execution gate: core converts relay-advertised wake cycle plans into capped prefetch execution plans with explicit profile-count, per-profile envelope, long-poll envelope, and total-cycle envelope limits. This item is closed. There is no standing "keep tuning OS behavior" task. Any future wake-related work must be opened as a new finite item that names exactly one platform, one trigger condition, one expected behavior, one acceptance test, and one stop condition.
