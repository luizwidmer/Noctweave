...
2. **Network anonymity remains out of scope**
   - Current: metadata reduction, replicated XOR-PIR primitives under a non-collusion assumption with padded fixed-bucket query shares, onion packet primitives, mixnet batch/cover scheduling, bounded continuous cover-cycle planning, deterministic diverse route selection, and mixnet route-policy validation.
