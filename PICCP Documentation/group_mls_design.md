...
- Group epoch replay is implemented in `GroupRatchetRecovery` and covered against stale serialized `GroupConversation` state, so the app and route tests use the same recovery path. Recovery replays retained epochs contiguously and fails closed when an intermediate epoch is no longer available.
...
- Route-level tests cover an offline member refreshing from a later signed epoch distribution, replaying multiple missed epoch distributions, two offline members independently replaying retained epoch history after a shared outage, decrypting a retained group attachment descriptor, retrieving/decrypting the relay attachment chunk after another member has already acknowledged the group envelope, and federated group-ratchet delivery from a sender's relay to the group-owning relay. Store-level coverage also verifies the relay keeps only the newest contiguous retained epoch history window.
