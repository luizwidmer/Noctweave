...
- ciphertext-only direct prefetch staging for app-intent or widget-triggered Apple sync paths using delegated inbox-access keys; helper config omits group routing metadata, and group helper fetch remains deferred until a non-identity delegated group credential exists
