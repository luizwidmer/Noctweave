...
- Relay-backed text group delivery uses signed group-ratchet envelopes stored in the group inbox; clients fetch, decrypt, and acknowledge those envelopes with member actor proofs.
- Group attachments and fallback/local-only group delivery still need the same group-ratchet transport treatment before the pairwise group path can be removed.
...
- **Deferred**: mixnet/onion transport, autonomous public DHT release mode, external audit, signed release-provenance packaging, group-ratchet attachment transport, and formal MLS-class proof work.

## Next Alignment Targets
- Prepare the external security-audit package.
- Extend group-ratchet relay delivery to attachments and remove the remaining pairwise fallback described in `group_mls_design.md`.
