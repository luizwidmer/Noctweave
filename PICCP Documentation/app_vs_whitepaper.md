...
- Open federation release profile based on coordinator snapshots, bounded peer exchange, and DHT gateway/native-overlay experiments, not autonomous public DHT participation. Discovery refreshes retain previously validated signed nodes across transient gateway or peer-query failures, and release verification fails if BEP5/libp2p/Kademlia autonomous adapter code appears in shipped source paths.
...
- No autonomous public DHT release mode; public-network adapters remain deferred until poisoning, churn, flooding, and operator-risk controls are externally validated. The local whitepaper verifier enforces this boundary by rejecting shipped source paths that introduce BEP5/libp2p/Kademlia adapter code.
...
- Retained group history coverage is complete for the current repository scope: tests cover multiple offline members recovering after a shared outage, fail-closed behavior after the retained epoch window expires, malformed history-chain rejection, mismatched retained-distribution metadata rejection, and deterministic fault injection for duplicate epochs, broken transcript links, and missing retained distributions. Actual device-lab fault injection is tracked separately only if device infrastructure is added.
- Open-federation work remains gated. Current simulation coverage includes feature-disabled fail-closed behavior, poisoned-node rejection, cached-node fallback after refresh failure, and stale-cache eviction on failed refresh. Public DHT adapters remain out of shipped source paths.
- Replicated XOR-PIR promotion is gated: optional support is not enough for a deployable claim unless fresh deployment evidence proves advertised replicas are available, match their advertised operators/endpoints, and carry unique non-collusion attestations.
