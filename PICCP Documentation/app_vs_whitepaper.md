...
- Core ciphertext prefetch can stage both direct-message envelopes and group-ratchet envelopes without decrypting or acknowledging them. The Apple closed-app helper currently publishes only delegated inbox-access keys, so helper fetches are limited to direct inbox ciphertext until group fetch has a delegated credential that is not the long-term identity signing key. The normal unlocked sync path remains responsible for group fetch, decryption, and clearing relay/staged records.
