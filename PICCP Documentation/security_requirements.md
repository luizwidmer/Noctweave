...
| SR-02 | Session establishment must resist harvest-now/decrypt-later attacks. | ML-KEM-768 prekey bundles and root-ratchet KEM steps. | prekey, bootstrap, and root-ratchet tests in `PICCPCoreTests.swift`. |
...
| SR-13 | Federation must not silently cross curated and open trust domains. | Federation mode policy, allow-list checks, coordinator quorum, and signed directory snapshots. | curated forwarding, strict policy, and coordinator tests. |
