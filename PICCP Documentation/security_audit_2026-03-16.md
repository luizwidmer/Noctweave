...
## Scope
- `PICCPCore` relay/client protocol paths
- Linux relay (`PICCP Relay Server`) TCP + HTTP/WebSocket bridge
...
- `swift test` passed in `PICCP Relay Server` (integration + parity tests).
- `swift test` passed in `PICCPCore`.
