...
private struct TestRunner {
    ...
    private mutating func testTwoClientRelayFlow() async {
        do {
            ...
            let requestData = try PICCPCoder.encode(deliverRequest)
            ...
            let decodedRequest = try PICCPCoder.decode(RelayRequest.self, from: requestData)
            ...
            let fetchData = try PICCPCoder.encode(fetchRequest)
            _ = try PICCPCoder.decode(RelayRequest.self, from: fetchData)
    ...
    private mutating func testQRCodeGeneration() {
        ...
        filter.message = Data("PICCP-UI-QR-Test".utf8)
    ...
    private mutating func testQRChunking() {
        let message = String(repeating: "PICCP", count: 400)
    ...
    private mutating func testRelayServerIntegrationIfAvailable() async {
        guard
            let host = ProcessInfo.processInfo.environment["PICCP_TEST_SERVER_HOST"],
            let portString = ProcessInfo.processInfo.environment["PICCP_TEST_SERVER_PORT"],
            let port = UInt16(portString)
        else {
            print("Skipping relay server integration test (set PICCP_TEST_SERVER_HOST/PORT).")
            return
        }
    ...
    private mutating func testRelayServerHealthIfAvailable() async {
        guard
            ...
            print("Skipping relay health test (set PICCP_TEST_SERVER_HOST/PORT).")
            return
        }
    ...
    private mutating func testInsecurePairingIfAvailable() async {
        guard
            ...
            print("Skipping insecure pairing test (set PICCP_TEST_SERVER_HOST/PORT).")
            return
        }
    ...
    private mutating func testAutoHealSessionMismatchIfAvailable() async {
        guard
            ...
            print("Skipping auto-heal test (set PICCP_TEST_SERVER_HOST/PORT).")
            return
        }
