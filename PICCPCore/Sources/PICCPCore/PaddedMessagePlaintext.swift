...
enum PaddedMessagePlaintext {
    ...
    static func encode(_ body: MessageBody) throws -> Data {
        let bodyData = try PICCPCoder.encode(body, sortedKeys: true)
    ...
    static func decode(_ data: Data) throws -> MessageBody {
        if data.count >= headerBytes,
           data.prefix(magic.count) == magic {
            ...
            return try PICCPCoder.decode(MessageBody.self, from: Data(data[bodyStart..<bodyEnd]))
        }
        return try PICCPCoder.decode(MessageBody.self, from: data)
    }
