...
enum LocalSecurityKeyPage {
    static let callbackScheme = "noctgallery-local-fido"

    static func html(options: Data, registration: Bool, token: String, nonce: String, brandImagePNG: Data? = nil) -> Data {
        ...
        <meta name="color-scheme" content="light dark"><title>Noct Gallery</title>
        <style nonce="\(nonce)">
        ...
        *{box-sizing:border-box}body{margin:0;min-height:100svh;display:grid;place-items:center;padding:24px;background:linear-gradient(145deg,#faf6f2,#f7eee9);font-size:17px;line-height:1.5}
        ...
        </style></head><body><main>\(brand)<p class="brand">Noct Gallery</p><div class="step" id="step">\(step)</div>
        ...
        async function complete(result){
          ...
          if(reply.next){
            ...
            document.querySelector('.intro').textContent='Your key was registered. Verify it once more to confirm that it can unlock Gallery.';
            status.textContent='Keep your key connected. This is the final check.';
            button.disabled=false;
          }else if(reply.complete){location.href='\(callbackScheme)://complete/\(token)';}
        ...
        button.onclick=async()=>{
          ...
          try{
            ...
            catch{status.textContent='This check has ended. Close this sheet and try again in Gallery.';}
          }
        };
        </script></body></html>
        """.utf8)
