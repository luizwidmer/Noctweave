...
swift test \
  ...
  --filter 'PICCPCoreTests/test(MetadataMinimizer|MessageEngineBuckets|GroupRatchetBuckets|HiddenRetrievalPlanner|HiddenRetrievalSupport|DecentralizedWakePlanner|OpenFederationDHTDiscoveryEngineFallsBack|OpenFederationDHTHTTPGatewayRefresh)'

echo "Verifying Linux relay open-federation parity coverage..."
swift test \
  --package-path "$ROOT_DIR/PICCP Relay Server" \
  --filter 'RelayStoreParityTests/test(HiddenRetrievalSupport|OpenFederationDHTHTTPGatewayRefresh|OpenFederationDHTNativeOverlay)'
